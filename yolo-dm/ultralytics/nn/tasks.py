...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, Detect_AFPN, Detect_MAFF, Detect_ASFF)):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in (
            ...
            args = [c1, c2, *args[1:]]
            if m in (BottleneckCSP, C1, C2, C2f, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3, C2f_DCNv4, C2f_UIB, C2f_GhostModule_DynamicConv, CSPPC, CSPStage):
        ...
        # ----------------------
            ...
            c2 = sum(ch[x] for x in f)
        elif m in (Detect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, Detect_AFPN, Detect_MAFF, Detect_ASFF):
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, Segment):
                ...
                return "obb"
            elif isinstance(m, (Detect, WorldDetect, Detect_AFPN, Detect_MAFF, Detect_ASFF)):
                return "detect"
